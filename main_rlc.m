...
clc;
format long;
global track_num noise_num L0 C R alpha u_record u_record_cmp coff_record t_record_cmp;
u_record = [];
u_record_cmp = [];
t_record_cmp = [];
coff_record = [];
...
switch track_num
    ...
    case 4
        trackname='expsin';
    case 5
        trackname='smooth-piecewise-linear(0/1/-1)';
end
noise_num = 3;
switch noise_num
    case 1
...
h_opt=odeset('RelTol',1e-10, 'AbsTol',1e-16, 'MaxStep',0.0001); 
[t,x]=ode15s(@ZPID_rlc,[0,T],[0 0 0], h_opt);
y = cal_x1noise(x(:,1), t, noise_num);
[t_cmp,x_cmp]=ode15s(@PID_rlc,[0,T],[0 0 0], h_opt);
y_cmp = cal_x1noise(x_cmp(:,1), t_cmp, noise_num);
...
for j=1:t_length
    [desired(j),yd1(j),yd2(j)]=caltrack_rlc(track_num,t(j));
    theta0(j) = cal_theta0(t(j));
end
for j=1:t_cmp_length
    [desired_cmp(j),yd1_cmp(j),yd2_cmp(j)]=caltrack_rlc(track_num,t_cmp(j));
end
error=x(:,1)-desired;
error_cmp=x_cmp(:,1)-desired_cmp;
disp(['MSE of ZPID:', num2str(sqrt((norm(error))^2/length(desired)))]);
disp(['MSE of PID:', num2str(sqrt((norm(error_cmp))^2/length(desired)))]);
% error_abs=abs(error);
% steady=1;
% for j=1:t_length
%     if error_abs(j)<10^(-5)
...
plot(t_cmp,x_cmp(:,1),'--b',t,x(:,1),'g',t,desired,'-.k','linewidth', 1);
...
% anno.HeadStyle = 'hypocycloid';
% anno.HeadWidth = 7;
legend('Z-PID','PID', Location="northeast");
xlabel('Time \rm(s)');
ylabel('Absolute tracking error (C)');
set(gca, 'FontName', 'times');
set(gca, 'fontsize', 12);
set(gcf,'Position', [500 250 550 325]);
% ylim([1e-15, 1e-5]);
...
u_pic = figure(4);
plot(t,u_record(1:t_length),'g',t_record_cmp, u_record_cmp,'--b', 'linewidth', 1);
% set(gca,'FontSize',FontSize);
legend('ZD-PID','ZD-PD');
xlabel('Time \rm(s)');
