...
[t_cmp,x_cmp]=ode15s(@PID_bai,[0,T], ori_cond, h_opt);
...
for j=1:t_length
    [desired(j),yd1(j),yd2(j)]=caltrack_bai(track_num,t(j));
    theta0(j) = cal_theta0(t(j));
end
for j=1:t_cmp_length
    [desired_cmp(j),yd1_cmp(j),yd2_cmp(j)]=caltrack_bai(track_num,t_cmp(j));
end
error=x(:,1)-desired;
...
% stem(9.5, 0.577778,'--',"Marker",'.','LineWidth',0.7,'MarkerSize',10, BaseValue=0);
% handle = legend('$y_{\mathrm{ZD-PD}}$','$y_{\mathrm{ZD-PID}}$', '$y_d$',Location='southeast');
% set(handle,'Interpreter','latex');
legend('Z-PID', 'PID','Desired',Location='southeast');
...
% anno.HeadStyle = 'hypocycloid';
% anno.HeadWidth = 7;
% anno = annotation('textarrow', [0.75, 0.85], [0.55, 0.68], 'String', '(9.5, 0.578) ');
% anno.FontName = 'times';
% anno.FontSize = 11;
% anno.HeadStyle = 'hypocycloid';
% anno.HeadWidth = 7;
